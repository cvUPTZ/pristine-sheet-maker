...
const MatchAnalysisV2: React.FC = () => {
  ...
  const fetchMatchDetails = useCallback(async () => {
    ...
    try {
      const {
        data: matchData,
        error: matchError
      } = await supabase.from('matches').select('home_team_name, away_team_name, home_team_formation, away_team_formation, home_team_players, away_team_players').eq('id', matchId).single();
      if (matchError) {
  ...
  const fetchTrackerAssignments = useCallback(async () => {
    ...
    try {
      console.log('Fetching tracker assignments for:', {
        matchId,
        userId: user.id
      });
      const {
        data,
        error
      } = await supabase.from('match_tracker_assignments').select('*').eq('match_id', matchId).eq('tracker_user_id', user.id);
      if (error) {
      ...
      const assignedEventTypesData: LocalEventType[] = eventTypes.filter(key => key).map(key => ({
        key,
        label: key
      }));
  ...
  const handleRecordEvent = async (eventTypeKey: string, playerId?: number, teamContext?: 'home' | 'away', details?: Record<string, any>) => {
    console.log("MatchAnalysisV2: handleRecordEvent called with:", {
      eventTypeKey,
      playerId,
      teamContext,
      details
    ...
    try {
      ...
      const {
        error: dbError
      } = await supabase.from('match_events').insert([eventToInsert]);
      if (dbError) {
  ...
  if (!matchId) {
    return <div className="flex items-center justify-center min-h-screen px-2 sm:px-4">
  ...
  return <div className="container mx-auto p-1 sm:p-2 lg:p-4 max-w-7xl">
      <div className="mb-3 sm:mb-4">
        <MatchHeader mode={mode} setMode={setMode} homeTeam={homeTeam} awayTeam={awayTeam} handleToggleTracking={handleToggleTracking} handleSave={handleSave} />
      </div>

      <Tabs defaultValue={defaultTab} className="w-full">
        
        
        {isAdmin && <TabsContent value="main" className="mt-2 sm:mt-4">
            <MainTabContentV2 matchId={matchId} homeTeam={homeTeam} awayTeam={awayTeam} isTracking={isTracking} onEventRecord={handleRecordEvent} />
          </TabsContent>}
        
        <TabsContent value="piano" className="mt-2 sm:mt-4">
          <div className="space-y-3 sm:space-y-4">
            {/* Voice Collaboration for Piano Input */}
            {user?.id && <VoiceCollaboration matchId={matchId} userId={user.id} />}
            ...
            <Card>
              <CardContent className="p-2 sm:p-3 lg:p-6">
                ...
                <TrackerPianoInput matchId={matchId} onRecordEvent={handleRecordEvent} />
              </CardContent>
            </Card>

            {/* Voice Input */}
            {assignedPlayers && assignedEventTypes && <Card>
                <CardContent className="p-2 sm:p-3 lg:p-6">
                  <h2 className="text-sm sm:text-base lg:text-lg font-semibold mb-2 sm:mb-3 lg:mb-4">
                    Voice Input
                  </h2>
                  <TrackerVoiceInput assignedPlayers={convertPlayersForVoiceInput(assignedPlayers)} assignedEventTypes={assignedEventTypes} onRecordEvent={handleRecordEvent} />
                </CardContent>
              </Card>}
          </div>
        </TabsContent>
        ...
        {isAdmin && <>
            <TabsContent value="planning" className="mt-2 sm:mt-4">
              <MatchPlanningNetwork matchId={matchId} width={isMobile ? 350 : 800} height={isMobile ? 400 : 600} />
            </TabsContent>
            ...
            <TabsContent value="tracker" className="mt-2 sm:mt-4">
              <Card>
                <CardContent className="p-2 sm:p-3 lg:p-6">
                  ...
                  <TrackerAssignment matchId={matchId} homeTeamPlayers={fullMatchRoster?.home || []} awayTeamPlayers={fullMatchRoster?.away || []} />
                </CardContent>
              </Card>
            </TabsContent>
          </>}
