...
const filterValidItems = <T extends { id: string }>(
  items: T[] | undefined,
  itemName: string
): T[] => {
  if (!items) return [];
...
const Admin: React.FC = () => {
  ...
  const [eventAssignments, setEventAssignments] = useState<
    UIDisplayedEventAssignment[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState("users");

  const [playerTrackerAssignments, setPlayerTrackerAssignments] = useState<
    UIDisplayedPlayerTrackerAssignment[]
  >([]);
  const [isCreateAssignmentDialogOpen, setIsCreateAssignmentDialogOpen] =
    useState(false);
  const [selectedMatchIdForAssignment, setSelectedMatchIdForAssignment] =
    useState<string | null>(null);
  const [selectedPlayerForAssignment, setSelectedPlayerForAssignment] =
    useState<{ playerId: number; playerTeamId: "home" | "away" } | null>(null);
  const [selectedTrackerIdForAssignment, setSelectedTrackerIdForAssignment] =
    useState<string | null>(null);
  ...
  const [
    isCreateEventAssignmentDialogOpen,
    setIsCreateEventAssignmentDialogOpen,
  ] = useState(false);
  const [
    selectedUserIdForEventAssignment,
    setSelectedUserIdForEventAssignment,
  ] = useState<string | null>(null);
  const [selectedEventTypeForAssignment, setSelectedEventTypeForAssignment] =
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: usersFunctionResponse, error: usersError } =
        await supabase.functions.invoke("get-all-users", { method: "GET" });

      if (usersError) {
        let errorMessage = usersError.message;
        if (
          usersError.context &&
          typeof usersError.context.error === "string"
        ) {
          errorMessage = usersError.context.error;
        } else if (
          usersError.data &&
          typeof usersError.data.error === "string"
        ) {
        ...
        Array.isArray(usersFunctionResponse)
      ) {
        ...
        toast.error(
          "Failed to process user data: unexpected format from function."
        );
        setUsers([]);
      }
      ...
      const { data: matchesData, error: matchesError } = await supabase
        .from("matches")
        .select(
          "id, name, match_type, description, home_team_name, away_team_name, status, match_date, created_at, home_team_players, away_team_players, home_team_formation, away_team_formation"
        )
      ...
      fetchedMatches = (matchesData || []).map((match) => {
        ...
        try {
          ...
          console.warn(
            `Failed to parse home_team_players for match ${match.id}:`,
            e
          );
        }
        try {
          ...
          console.warn(
            `Failed to parse away_team_players for match ${match.id}:`,
            e
          );
        }
        return {
      ...
      const { data: eventAssignmentsData, error: eventAssignmentsError } =
        await supabase
      ...
      if (eventAssignmentsData) {
        const augmentedEventAssignments = eventAssignmentsData.map(
          (assignment: any) => {
        ...
        setEventAssignments(
          augmentedEventAssignments as UIDisplayedEventAssignment[]
        );
      } else {
        setEventAssignments([]);
      }

      const { data: rawAssignments, error: playerTrackerAssignmentsError } =
        await supabase
          .from("match_tracker_assignments")
          .select(
            "id, match_id, tracker_user_id, player_id, player_team_id, created_at"
          ); // Ensure player_id is selected
      if (playerTrackerAssignmentsError) throw playerTrackerAssignmentsError;

      if (rawAssignments) {
        const processedAssignments = rawAssignments
          .map((assignment) => {
            // Skip if player_id is null or undefined for Player-Tracker Assignments table
            if (
              assignment.player_id === null ||
              assignment.player_id === undefined
            ) {
              return null; // This assignment is not for a specific player, might be general match tracker
            }

            const match = fetchedMatches.find(
              (m) => m.id === assignment.match_id
            );
            const tracker = fetchedUsers.find(
              (u) => u.id === assignment.tracker_user_id
            );
            ...
            if (match) {
              const playerList =
                assignment.player_team_id === "home"
                  ? match.home_team_players
                  : match.away_team_players;
              // Ensure player_id is treated as a number for comparison with Player.id
              const player = Array.isArray(playerList)
                ? playerList.find((p) => p.id === Number(assignment.player_id))
                : null;
            ...
            return {
              ...
              trackerName:
                tracker?.full_name || tracker?.email || "Unknown Tracker",
              created_at: assignment.created_at,
            };
          })
          .filter(Boolean); // Filter out nulls (general match assignments)
        setPlayerTrackerAssignments(
          processedAssignments as UIDisplayedPlayerTrackerAssignment[]
        );
      } else {
        setPlayerTrackerAssignments([]);
      }
  ...
  const handleCreateEventAssignment = async () => {
    ...
    try {
      ...
      if (insertError || !newAssignmentData)
        throw (
          insertError || new Error("No data returned from assignment creation")
        );
  ...
  const handleCreatePlayerAssignment = async () => {
    if (
      ...
      !selectedTrackerIdForAssignment
    ) {
    ...
    try {
      ...
      if (error || !newAssignmentData)
        throw (
          error || new Error("No data returned from player assignment creation")
        );
      const match = matches.find((m) => m.id === newAssignmentData.match_id);
      const tracker = users.find(
        (u) => u.id === newAssignmentData.tracker_user_id
      );
      let playerName = "Unknown Player";
      let playerTeamName = "Unknown Team";
      if (match) {
        const playerList =
          newAssignmentData.player_team_id === "home"
            ? match.home_team_players
            : match.away_team_players;
        const player = Array.isArray(playerList)
          ? playerList.find((p) => p.id === Number(newAssignmentData.player_id))
          : undefined;
        if (player) playerName = player.name;
        playerTeamName =
          newAssignmentData.player_team_id === "home"
      ...
      setPlayerTrackerAssignments((prev) =>
        [newlyConstructedAssignment, ...prev].sort(
          (a, b) =>
  ...
  const handleDeletePlayerAssignment = async (assignmentId: string) => {
    ...
    if (
      !confirm(
        "Are you sure you want to delete this player-tracker assignment?"
      )
    )
      return;
    try {
  ...
  const handleCreateUser = async () => {
    ...
    if (!newUserName || !newUserEmail || !newUserPassword) {
      toast.error(
        "Please fill in all required fields: Full Name, Email, and Password."
      );
      return;
    }
    try {
  ...
  const handleRoleChange = async (userId: string, newRole: UserRole) => {
    ...
    try {
      ...
      if (error) {
        console.error(
          "Raw error object from invoke admin-set-user-role:",
          error
        );
        const message =
          error.context?.errorMessage ||
          error.message ||
          "Unknown error from function invoke.";
      ...
      if (currentUser && currentUser.id === userId) {
        console.warn(
          "Current user's role updated. Session refresh might be needed for claims to update immediately."
        );
      }
    } catch (e: any) {
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    if (
      !confirm(
        "Are you sure you want to delete this user? This involves multiple steps and might be irreversible."
      )
    )
      return;
    try {
      ...
      if (error) throw error;
      toast.success(
        "User deletion process initiated. Full cleanup depends on backend implementation."
      );
      await fetchData(); // Refetch users
    } catch (e: any) {
  ...
  const handleDeleteMatch = async (matchId: string) => {
    ...
    if (
      !confirm(
        "Are you sure you want to delete this match? This will also delete related assignments."
      )
    )
      return;
    try {
      ...
      if (assignmentError) {
        toast.warning(
          `Could not clear assignments for match, but proceeding with match deletion: ${assignmentError.message}`
        );
      }
  ...
  const validMatchesForSelection = React.useMemo(
    () => filterValidItems(matches, "match"),
    [matches]
  );
  const validUsersForSelection = React.useMemo(
    () => filterValidItems(users, "user"),
    [users]
  );
  const validTrackersForSelection = React.useMemo(
    () =>
      filterValidItems(
        users.filter((u) => u.role === "tracker"),
        "tracker"
      ),
    [users]
  );
  ...
  if (loading) {
    return (
      <div className="container mx-auto p-6 flex items-center justify-center h-64 text-lg">
        Loading admin data...
      </div>
    );
  }
    ...
    <div className="container mx-auto p-6">
                ...
                <TableBody>
                  {matches.map((match) => (
                    <TableRow key={match.id}>
                      <TableCell className="font-medium">
                        {match.name || match.description || "Unnamed Match"}
                      </TableCell>
                      <TableCell>
                        {match.match_type
        ...
        <TabsContent value="assignments" className="mt-6">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button
                onClick={() => {
                  setIsCreateEventAssignmentDialogOpen(true);
                  resetCreateEventAssignmentForm();
                }}
              >
                Assign Event Type to User
              </Button>
            </CardHeader>
            <CardContent>
              <Table>
                <TableCaption>
                  User assignments for specific event types.
                </TableCaption>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableRow key={assignment.id}>
                      ...
                      <TableCell>
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleRemoveEventAssignment(assignment.id)
                          }
                        >
                          Remove
                        </Button>
                    ...
                    <TableRow key={assignment.id}>
                      ...
                      <TableCell>
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleDeletePlayerAssignment(assignment.id)
                          }
                        >
                          Delete
                        </Button>
      ...
      >
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Create Player-Tracker Assignment</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  setSelectedMatchIdForAssignment(value);
                  setSelectedPlayerForAssignment(null);
                }}
              >
                ...
                <SelectContent>
                  {validMatchesForSelection.length > 0 ? (
                    validMatchesForSelection.map((match) => (
            ...
            <div>
              ...
              <Select
                value={
                  selectedPlayerForAssignment
                    ? `${selectedPlayerForAssignment.playerId}:${selectedPlayerForAssignment.playerTeamId}`
                    : ""
                }
                onValueChange={(value) => {
                  if (value) {
                    const [idStr, team] = value.split(":");
                    setSelectedPlayerForAssignment({
                      playerId: parseInt(idStr),
                      playerTeamId: team as "home" | "away",
                    });
                  } else {
                    setSelectedPlayerForAssignment(null);
                  }
                  ...
                  {selectedMatchIdForAssignment &&
                    (() => {
                      const currentMatch = matches.find(
                        (m) => m.id === selectedMatchIdForAssignment
                      );
                      if (!currentMatch)
                        return (
                      ...
                      (Array.isArray(currentMatch.home_team_players)
                        ? currentMatch.home_team_players
                        : []
                      ).forEach((p) => {
                        if (p && p.id !== undefined && p.id !== null && p.name)
                          players.push(
                            <SelectItem
                              ...
                              value={`${p.id}:home`}
                            >
                              {p.name} ({currentMatch.home_team_name} - Home)
                            </SelectItem>
                          );
                      });
                      (Array.isArray(currentMatch.away_team_players)
                        ? currentMatch.away_team_players
                        : []
                      ).forEach((p) => {
                        if (p && p.id !== undefined && p.id !== null && p.name)
                          players.push(
                            <SelectItem
                              ...
                              value={`${p.id}:away`}
                            >
                              {p.name} ({currentMatch.away_team_name} - Away)
                            </SelectItem>
                          );
                      });
                      return players.length ? (
                        ...
                        <div className="px-2 py-1.5 text-sm text-muted-foreground">
                          No players in this match. Ensure players have 'id' and
                          'name'.
                        </div>
                      );
                    })()}
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={setSelectedTrackerIdForAssignment}
              >
                <SelectTrigger id="assign-tracker">
                  <SelectValue placeholder="Select a tracker" />
                </SelectTrigger>
                <SelectContent>
                  {validTrackersForSelection.length > 0 ? (
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreatePlayerAssignment}>Create</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent className="sm:max-w-2xl">
          {" "}
          {/* Adjusted width for better form display */}
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          {editingMatch && (
            <CreateMatchForm
              isEditMode
              initialData={{
                // Map fields from `Match` to `MatchFormData`
                id: editingMatch.id,
      ...
      <Dialog
        open={isCreateEventAssignmentDialogOpen}
        onOpenChange={setIsCreateEventAssignmentDialogOpen}
      >
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              >
                ...
                <SelectContent>
                  {validUsersForSelection.length > 0 ? (
                    validUsersForSelection.map((user) => (
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setSelectedEventTypeForAssignment(v as EventType)
                }
              >
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setIsCreateEventAssignmentDialogOpen(false)}
            >
              Cancel
            </Button>
            <Button onClick={handleCreateEventAssignment}>Assign</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
