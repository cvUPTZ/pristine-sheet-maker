...
interface WebRTCManagerCallbacks {
  ...
  onReadyForInteraction?: () => void; // New callback
}
...
export class WebRTCManager {
  ...
  public onReadyForInteraction: WebRTCManagerCallbacks['onReadyForInteraction'];

  private signalingReady = false;
  private presenceReadyAndSynced = false;
  private interactionReadyCalled = false;
  ...
  constructor(
    ...
    if (callbacks) {
      ...
      this.onReadyForInteraction = callbacks.onReadyForInteraction; // Assign new callback
    }
  ...
  public async joinRoom(roomId: string): Promise<void> {
    ...
    this.resetReadyFlags(); // Reset flags for a new room join
    ...
    try {
      ...
      this.signalingChannel
        ...
        .subscribe((status, err) => {
          if (status === 'SUBSCRIBED') {
            ...
            this.checkIfReadyForInteraction();
            ...
            this.resetReadyFlags(); // Signaling failed, cannot be ready
          } else if (status === 'CLOSED') { // Channel was closed
      ...
      this.presenceChannel.on('presence', { event: 'sync' }, () => {
        ...
        let initialParticipantsProcessed = false;
        if (presenceState) {
          const remoteUserIds = Object.keys(presenceState).filter(id => id !== this.localUserId);
          console.log(`Presence sync: found ${remoteUserIds.length} remote users.`);
          remoteUserIds.forEach(pId => {
             // @ts-ignore TODO: state structure might need casting
            const presences = presenceState[pId] as Array<{ user_name?: string }>;
            const pName = presences[0]?.user_name;
            this.createPeerConnection(pId, pName || `User ${pId}`, true);
          });
          initialParticipantsProcessed = true;
        } else {
          // No other users, or presence state not yet available.
          // Still ready if signaling is up.
          initialParticipantsProcessed = true;
        }

        // If signaling is also subscribed, then we are ready.
        if (initialParticipantsProcessed) { // True even if no other users, means sync completed
            this.presenceReadyAndSynced = true;
            this.checkIfReadyForInteraction();
        }
      });
      ...
      this.presenceChannel.subscribe(async (status, err) => {
        if (status === 'SUBSCRIBED') {
          ...
          // Consider calling onReadyForInteraction here if not already called by sync,
          // specifically if this is the first successful subscription event.
          // However, sync event is more reliable for initial participant setup.
          // If signaling channel also joined, trigger ready.
          // No specific onReadyForInteraction logic here, success of presence subscription itself doesn't mean synced.
          // The 'sync' event is the key for presence readiness.
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
            console.error(`Presence channel subscription error for room ${roomId}:`, err);
            this.onError?.('presence_subscribe_error', err);
            this.resetReadyFlags(); // Presence failed, cannot be ready
  ...
  private resetReadyFlags(): void {
    ...
    this.presenceReadyAndSynced = false;
    this.interactionReadyCalled = false;
  }

  private checkIfReadyForInteraction(): void {
    if (this.signalingReady && this.presenceReadyAndSynced && !this.interactionReadyCalled) {
      console.log('WebRTCManager is now ready for interaction.');
      this.interactionReadyCalled = true; // Ensure it's called only once
      this.onReadyForInteraction?.();
    }
  }
  ...
  private handleSignalingMessage(message: /* RealtimePostgresChangesPayload<any>['new'] */ any): void {
    ...
    switch (messageType) { // Using message.message_type as the switch key
      ...
      case 'participant-mute-status':
        // data is expected to be { isMuted: boolean }
        // The sender_id of the message is the authoritative userId for this status.
        if (typeof data.isMuted === 'boolean') {
          this.onParticipantMuteChanged?.(fromUserId, data.isMuted); // Use fromUserId (which is message.sender_id)
        } else {
          console.warn('Malformed participant-mute-status message (missing isMuted):', message);
        }
        break;
      case 'admin-set-mute-request':
        // Expected payload: { muted: boolean, moderatorId: string }
        // This message is targeted, so to_user_id should be this.localUserId (already filtered by joinRoom subscription logic)
        if (message.to_user_id === this.localUserId && typeof data.muted === 'boolean') {
          console.log(`Admin command received: ${data.muted ? 'Mute' : 'Unmute'} self. Moderator: ${data.moderatorId || 'Unknown'}`);
          if (this.localStream) {
            let actualNewMuteState: boolean | undefined;
            this.localStream.getAudioTracks().forEach(track => {
              track.enabled = !data.muted; // track.enabled is true if unmuted
              actualNewMuteState = !track.enabled;
            });

            if (typeof actualNewMuteState === 'boolean') {
              // Inform local UI/hook about the change (admin-initiated)
              this.onParticipantMuteChanged?.(this.localUserId, actualNewMuteState);

              // Broadcast the new actual mute status to others
              this.sendSignalingMessage(null, 'participant-mute-status', {
                userId: this.localUserId,
                isMuted: actualNewMuteState,
              });
              // Optionally, trigger a more specific callback if UI needs to know it was an admin action
              // this.onAdminMuted?.(data.moderatorId, actualNewMuteState);
            }
          } else {
            console.warn('Received admin-set-mute-request but no local stream to apply to.');
          }
        } else if (message.to_user_id !== this.localUserId) {
          // This should not happen if RLS and subscription filters are correct
          console.warn(`Received admin-set-mute-request not intended for local user: ${message.to_user_id}`);
        } else {
            console.warn('Malformed admin-set-mute-request message:', message);
        }
        break;
      default:
  ...
  public sendAdminMuteCommand(targetUserId: string, mute: boolean): void {
    if (!this.currentRoomId) {
      console.error('Cannot send admin mute command, not in a room.');
      this.onError?.('admin_command_error', new Error('Not in a room'));
      return;
    }
    console.log(`Sending admin mute command to ${targetUserId}, mute: ${mute}`);
    this.sendSignalingMessage(targetUserId, 'admin-set-mute-request', {
      muted: mute,
      moderatorId: this.localUserId, // The admin sending the command
    });
  }
  ...
  public async leaveRoom(): Promise<void> {
    ...
    this.currentRoomId = null;
    this.resetReadyFlags(); // Reset flags when leaving room
  ...
  public async toggleMute(): Promise<boolean | undefined> {
    ...
    if (typeof newMuteState === 'boolean' && this.currentRoomId) {
      ...
      await this.sendSignalingMessage(null, 'participant-mute-status', {
        // userId: this.localUserId, // No longer needed in payload, sender_id is used by receiver
