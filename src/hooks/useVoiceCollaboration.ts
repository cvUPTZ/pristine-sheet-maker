...
import { PRODUCTION_VOICE_CONFIG } from '../../config/voiceConfig'; // Import ICE server config
...
export const useVoiceCollaboration = ({
  ...
  useEffect(() => {
    ...
    const iceServersConfig = PRODUCTION_VOICE_CONFIG.iceServers;
    // Potentially add TURN servers from env vars here if found
    // Example:
    // const turnUrl = import.meta.env.VITE_TURN_URL;
    // const turnUsername = import.meta.env.VITE_TURN_USERNAME;
    // const turnCredential = import.meta.env.VITE_TURN_CREDENTIAL;
    // if (turnUrl && turnUsername && turnCredential) {
    //   iceServersConfig.push({ urls: turnUrl, username: turnUsername, credential: turnCredential });
    // }
    
    const manager = new WebRTCManager(supabase, userId, userName, iceServersConfig, {
      ...
      onLocalStreamReady: (stream) => {
        ...
        // Potentially set a state here if UI needs to react to local stream being available
      },
      onReadyForInteraction: () => {
        console.log('[Hook] WebRTCManager ready for interaction.');
        setConnectionState('connected');
        setIsConnected(true);
        setIsConnecting(false);
        setIsVoiceEnabled(true);
        // Update current user details now that we are fully ready
  ...
  const joinVoiceRoom = useCallback(async (roomToJoin: VoiceRoom): Promise<void> => {
    ...
    try {
      ...
      // States like isConnected, isVoiceEnabled will now be primarily set by onReadyForInteraction
      // setCurrentUser can also be moved to onReadyForInteraction to ensure manager is ready for getLocalMuteState

      // No longer setting optimistic isConnected/isVoiceEnabled here.
      // setCurrentUser will be set in onReadyForInteraction.
      toast.success(`Attempting to join voice room: ${roomToJoin.name}...`);
  ...
  const adminSetParticipantMute = useCallback(async (participantId: string, mutedState: boolean): Promise<void> => {
    ...
    if (webRTCManagerRef.current) {
      webRTCManagerRef.current.sendAdminMuteCommand(participantId, mutedState);
      toast.info(`Admin command sent to ${mutedState ? 'mute' : 'unmute'} ${participantId}.`);
    } else {
      toast.error('WebRTC Manager not available to send admin command.');
      console.error('WebRTC Manager not available to send admin command.');
    }
    // The participant's mute state will update via the onParticipantMuteChanged callback
    // after the target client processes the admin command and broadcasts their new status.
