  ...
  const fetchMatch = async () => {
    ...
    try {
      ...
      const matchData: MatchState = {
        ...
        venue: data.location || undefined, // Map location to venue
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          home_team_players: JSON.stringify(updates.homeTeamPlayers || updates.home_team_players),
          away_team_players: JSON.stringify(updates.awayTeamPlayers || updates.away_team_players),
          ...
          location: updates.venue, // Map venue to location
          competition: updates.competition,
          match_statistics: updates.statistics || updates.match_statistics,
          ball_tracking_data: JSON.stringify(updates.ballTrackingData || updates.ball_tracking_data),
        })
        .eq('id', matchId);
